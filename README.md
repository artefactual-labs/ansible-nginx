...
You can use this option when not using the wildcard servername in your sites (`servername _`).

Just set as `True` the `nginx_create_default_deny_all_site` variable (default False). Then the role will create
a site with (and it's self-signed certificate):
...
server {
    ...
    ssl_certificate /etc/nginx/conf.d/default-deny-all.crt;
    ssl_certificate_key /etc/nginx/conf.d/default-deny-all-key.crt;
...
All these requests to non configured virtualhosts are going to be denied with 403 status code (http) or invalid 
cypher (https). This new site will not send any line to logs (access or error). 
